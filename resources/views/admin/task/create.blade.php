...
<body>
    ...
    <div class="head-box">
        <h1>Create Task in {{ $project->name }}</h1>
    </div>
    <div class="container-fluid overflow-y-auto">
        ...
        <form action="{{ route('store.task') }}" method="POST">
            ...
            <div class="mb-3">
                <input type="text" class="form-control" id="name" name="project_id" value="{{ $project->id }}"
                    hidden>
            </div>
    
            <div class="mb-3">
                <label for="name" class="form-label">Assign To</label>
                <select id="name" class="form-select" aria-label="Default select example" name="user_id">
            ...
            <div class="row g-3">
                <div class="col">
                    ...
                    <input type="date" id="start_date" class="form-control" placeholder="" min="<?php echo date('Y-m-d'); ?>"
                        name="started_at">
                </div>
                <div class="col">
                    <label for="end_date" class="form-label">Project End Date</label>
                    <input type="date" id="end_date" class="form-control" placeholder="" min="<?php echo date('Y-m-d'); ?>"
                        name="end_at">
                </div>
            </div>
